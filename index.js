...
async function main() {
  ...
  let startIndex = prompt("请输入起始章节（1）: ");
  let endIndex = prompt("请输入结束章节（2000）: ");
  if (startIndex == "") startIndex = 1;
  if (endIndex == "") endIndex = 2000;

  let dir = prompt(`请输入小说存放资料夹(预设: ${defaultSaveDir})/书名: `);
  if (dir == "") {
    dir = defaultSaveDir;
  }
  ...
  bookSourceName = prompt("请指定书源文件, 不指定则根据url自动匹配: ");
